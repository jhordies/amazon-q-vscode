...
exports.activateAnthropicServer = exports.AnthropicCompatServer = exports.execInContainer = void 0;
const http = __importStar(require("http"));
const vscode = __importStar(require("vscode"));
const codewhisperer_1 = require("aws-core-vscode/codewhisperer");
...
const environmentStore = new Map();
...
function anthropicContentToOpenAI(content) {
    ...
    for (const block of content) {
        if (block.type === 'text') {
            ...
            tool_calls.push({
                id: block.id ?? (0, crypto_1.randomUUID)(),
                type: 'function',
                function: {
                    name: block.name ?? '',
...
function anthropicMessagesToOpenAI(messages, systemPrompt) {
    ...
    for (const m of messages) {
        if (m.role === 'system') {
            ...
            result.push({ role: 'system', content: typeof m.content === 'string' ? m.content : (0, serverUtils_1.extractText)(m.content) });
            continue;
        }
...
function anthropicToolsToOpenAI(tools) {
    return tools
        ...
        function: {
            ...
            parameters: (0, serverUtils_1.sanitizeSchema)(t.input_schema ?? {}),
        },
    }));
}
...
function notSupported(res, feature) {
    anthropicError(res, 501, 'not_supported_error', `${feature} requires Docker. Enable it in Amazon Q settings (amazonQ.anthropicServer.dockerEnabled) and ensure Docker is running.`);
}
 ...
 */
async function executeMessages(req) {
    ...
    const sessionId = serverUtils_1.sessionStore.create(openaiMessages, model, openaiTools);
    const sessionKey = (0, serverUtils_1.buildSessionKey)(openaiMessages);
    const prevState = serverUtils_1.convStateMap.get(sessionKey) ?? serverUtils_1.sessionStore.get(sessionId)?.convState;
    const trimmed = (0, serverUtils_1.trimMessages)(openaiMessages, model, prevState);
    oaiReq.messages = trimmed;
    const conversationId = (0, crypto_1.randomUUID)();
    let profileArn;
    try {
        profileArn = codewhisperer_1.AuthUtil.instance.regionProfileManager?.activeRegionProfile?.arn;
    }
    catch { /* optional */ }
    const payload = (0, serverUtils_1.buildKiroPayload)(oaiReq, conversationId, profileArn);
    if (req.max_tokens)
        payload.conversationState.currentMessage.userInputMessage.maxTokens = req.max_tokens;
    const upstream = await (0, serverUtils_1.streamFromCW)(payload);
    ...
    for await (const raw of upstream) {
        buffer.value += raw.toString('utf-8');
        for (const ev of (0, serverUtils_1.parseChunk)(buffer)) {
            if (ev.type === 'content') {
                ...
                currentTool = {
                    id: ev.data.toolUseId ?? `toolu_${(0, crypto_1.randomUUID)().slice(0, 8)}`,
                    name: ev.data.name ?? '',
                ...
                try {
                    currentTool.input = JSON.parse(currentTool._rawArgs);
                }
                catch { /* keep partial */ }
                toolCalls.push(currentTool);
                currentTool = null;
                ...
                const pct = ev.data.contextUsagePercentage ?? 0;
                const state = serverUtils_1.convStateMap.get(sessionKey) ?? { contextUsagePct: 0 };
                state.contextUsagePct = pct;
                serverUtils_1.convStateMap.set(sessionKey, state);
                serverUtils_1.sessionStore.updateConvState(sessionId, { contextUsagePct: pct });
            }
        }
    }
    if (currentTool) {
    ...
    if (completionTokens === 0) {
        const outputChars = fullContent.length + dedupedTools.reduce((s, tc) => s + JSON.stringify(tc.input).length, 0);
    ...
    const msgId = `msg_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
    return {
        id: msgId,
        type: 'message',
        role: 'assistant',
...
// ── POST /v1/messages ─────────────────────────────────────────────────────────
async function handleMessages(req, res, incomingHeaders) {
    const model = req.model ?? 'claude-sonnet-4.5';
    const systemPrompt = extractSystemPrompt(req.system);
    const openaiMessages = anthropicMessagesToOpenAI(req.messages, systemPrompt);
    const openaiTools = req.tools?.length ? anthropicToolsToOpenAI(req.tools) : undefined;
    // Session management (same pattern as OpenAI server)
    const incomingSessionId = incomingHeaders['x-session-id'];
    let sessionId;
    let effectiveMessages;
    if (incomingSessionId) {
        const session = serverUtils_1.sessionStore.get(incomingSessionId);
        if (!session) {
        ...
        const merged = serverUtils_1.sessionStore.append(incomingSessionId, openaiMessages, openaiTools);
        effectiveMessages = merged;
        sessionId = incomingSessionId;
    }
    else {
        sessionId = serverUtils_1.sessionStore.create(openaiMessages, model, openaiTools);
        effectiveMessages = openaiMessages;
    }
    const sessionKey = (0, serverUtils_1.buildSessionKey)(effectiveMessages);
    const prevState = serverUtils_1.convStateMap.get(sessionKey) ?? serverUtils_1.sessionStore.get(sessionId)?.convState;
    if (prevState?.contextUsagePct !== undefined && prevState.contextUsagePct >= 90) {
        ...
        prevState.summary = lastAssistant
            ? `Last assistant response: ${(0, serverUtils_1.extractText)(lastAssistant.content).slice(0, 2000)}`
            : 'Context was compressed due to length.';
        prevState.contextUsagePct = 0;
    }
    const trimmed = (0, serverUtils_1.trimMessages)(effectiveMessages, model, prevState);
    const oaiReq = { model, messages: trimmed, tools: openaiTools, stream: req.stream, max_tokens: req.max_tokens };
    const conversationId = (0, crypto_1.randomUUID)();
    const msgId = `msg_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
    let profileArn;
    try {
    ...
    let upstream;
    try {
        upstream = await (0, serverUtils_1.streamFromCW)(payload);
    }
    catch (err) {
        serverUtils_1.log.error('anthropicServer: CW request failed: %s', err);
    ...
    if (req.stream) {
        ...
        for await (const raw of upstream) {
            ...
            for (const ev of (0, serverUtils_1.parseChunk)(buffer)) {
                if (ev.type === 'content') {
                    ...
                    if (currentTool)
                        toolCalls.push(currentTool);
                    const toolId = ev.data.toolUseId ?? `toolu_${(0, crypto_1.randomUUID)().slice(0, 8)}`;
        ...
        if (completionTokens === 0) {
            const outputChars = streamedContent.length + toolCalls.reduce((s, tc) => s + JSON.stringify(tc.input).length, 0);
            completionTokens = Math.ceil(outputChars / 4);
        }
        ...
        const assistantMsg = { role: 'assistant', content: streamedContent || null };
        if (toolCalls.length) {
        ...
        serverUtils_1.sessionStore.append(sessionId, [assistantMsg]);
    }
    else {
        // ── Non-streaming ─────────────────────────────────────────────────────
        let fullContent = '';
        ...
        for (const tc of toolCalls) {
            ...
            if (!ex || JSON.stringify(tc.input).length > JSON.stringify(ex.input).length)
                seen.set(tc.id, tc);
        }
        ...
        if (completionTokens === 0) {
            const outputChars = fullContent.length + deduped.reduce((s, tc) => s + JSON.stringify(tc.input).length, 0);
            completionTokens = Math.ceil(outputChars / 4);
        }
        res.writeHead(200, {
            ...
            'X-Session-Id': sessionId,
            'anthropic-version': '2023-06-01',
            'request-id': (0, crypto_1.randomUUID)(),
        });
...
async function processBatch(batchId, requests) {
...
function handleBatches(method, pathParts, body, res) {
    ...
    if (method === 'POST' && !batchId) {
        ...
        const id = `msgbatch_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
...
async function handleFiles(method, pathParts, req, res) {
    ...
    if (method === 'POST' && !fileId) {
        ...
        const { fields, files } = (0, serverUtils_1.parseMultipart)(body, boundaryMatch[1]);
        if (!files.length) {
            anthropicError(res, 400, 'invalid_request_error', 'No file in request');
            return;
        }
        const f = files[0];
        const id = `file_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
        const stored = {
...
// ── Models API ────────────────────────────────────────────────────────────────
async function handleModels(method, pathParts, res) {
    const modelId = pathParts[3]; // /v1/models/:id  (parts: ['','v1','models',':id'])
    const created = Math.floor(Date.now() / 1000);
    const models = await (0, serverUtils_1.fetchModelList)();
...
// ── Skills API ────────────────────────────────────────────────────────────────
function handleSkills(method, pathParts, body, res) {
    ...
    if (method === 'POST' && !skillId) {
        ...
        const id = `skill_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
        const skill = { id, name: body.name, description: body.description, input_schema: body.input_schema, created_at: now, updated_at: now };
...
// ── Agents API ────────────────────────────────────────────────────────────────
function handleAgents(method, pathParts, body, res) {
    ...
    if (method === 'POST' && !agentId) {
        ...
        const id = `agent_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
        const agent = { id, name: body.name, description: body.description, model: body.model, system_prompt: body.system_prompt, skill_ids: body.skill_ids ?? [], created_at: now, updated_at: now };
...
// ── Environments API ──────────────────────────────────────────────────────────
function handleEnvironments(method, pathParts, body, res) {
    ...
    if (method === 'POST' && !envId) {
        ...
        const id = `env_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
        const env = {
            id, name: body.name,
            image: body.image ?? vscode.workspace.getConfiguration('amazonQ').get('anthropicServer.defaultEnvironmentImage', 'ubuntu:24.04'),
            memory_mb: body.memory_mb ?? vscode.workspace.getConfiguration('amazonQ').get('anthropicServer.containerMemoryMb', 512),
...
async function startContainer(image, memoryMb, sessionId) {
...
async function execInContainer(containerId, command) {
    try {
        const { stdout, stderr } = await execFileAsync('docker', ['exec', containerId, 'sh', '-c', command], { timeout: 30000 });
...
// ── Sessions API ──────────────────────────────────────────────────────────────
async function handleSessions(method, pathParts, body, req, res) {
    const cfg = vscode.workspace.getConfiguration('amazonQ');
    const dockerEnabled = cfg.get('anthropicServer.dockerEnabled', false);
    ...
    if (method === 'POST' && !sessionId) {
        ...
        const envId = body.environment_id;
        const agent = agentId ? agentStore.get(agentId) : undefined;
        const env = envId ? environmentStore.get(envId) : undefined;
        const image = env?.image ?? cfg.get('anthropicServer.defaultEnvironmentImage', 'ubuntu:24.04');
        const memoryMb = env?.memory_mb ?? cfg.get('anthropicServer.containerMemoryMb', 512);
        if (!(await isDockerAvailable())) {
            anthropicError(res, 503, 'api_error', 'Docker is not available or not running.');
            return;
        }
        const id = `sess_${(0, crypto_1.randomUUID)().replace(/-/g, '').slice(0, 24)}`;
        const tools = [];
        ...
        if (agent?.skill_ids?.length) {
            for (const sid of agent.skill_ids) {
                ...
                if (skill)
                    tools.push({ type: 'function', function: { name: skill.name, description: skill.description, parameters: skill.input_schema } });
            }
        }
        const session = {
        ...
        try {
            ...
            serverUtils_1.log.error('anthropicServer: failed to start container: %s', err);
        }
        res.writeHead(200, { 'Content-Type': 'application/json', 'anthropic-version': '2023-06-01' });
...
class AnthropicCompatServer {
    ...
    async start() {
        ...
        const srv = http.createServer(async (req, res) => {
            ...
            try {
                ...
                if (pathname === '/v1/messages/count_tokens' && method === 'POST') {
                    if (!codewhisperer_1.AuthUtil.instance.isConnected()) {
                        anthropicError(res, 401, 'authentication_error', 'Not authenticated with Amazon Q');
                        return;
                    }
                    const raw = await (0, serverUtils_1.readBody)(req);
                    let parsed;
                    try {
                ...
                serverUtils_1.log.error('anthropicServer: unhandled error: %s', err);
                if (!res.headersSent)
        ...
        return new Promise((resolve, reject) => {
            srv.listen(this._port, '127.0.0.1', () => {
                this.server = srv;
                serverUtils_1.log.info('Anthropic-compatible server listening on http://127.0.0.1:%d', this._port);
                resolve();
            });
            srv.on('error', (err) => {
    ...
    stop() {
        ...
        return new Promise((resolve) => {
            ...
            this.server.close(() => { this.server = undefined; serverUtils_1.log.info('Anthropic-compatible server stopped'); resolve(); });
        });
    }
}
exports.AnthropicCompatServer = AnthropicCompatServer;
// ── Settings webview ─────────────────────────────────────────────────────────
function buildAnthropicSettingsHtml(panel, running, port, autoStart, dockerEnabled, dockerImage, containerMemoryMb, retryOnPortBusy, retryIntervalSeconds) {
...
// ── Activation ────────────────────────────────────────────────────────────────
let anthropicServerInstance;
let anthropicSettingsPanel;
function pushAnthropicSettingsState(running, port) {
    anthropicSettingsPanel?.webview.postMessage({ command: 'stateUpdate', running, port });
}
function activateAnthropicServer(context) {
    const config = vscode.workspace.getConfiguration('amazonQ');
    const port = config.get('anthropicServer.port', 61823);
    const autoStart = config.get('anthropicServer.autoStart', true);
    anthropicServerInstance = new AnthropicCompatServer(port);
    context.subscriptions.push(vscode.commands.registerCommand('aws.amazonq.anthropicServer.start', async () => {
        // Re-read config so a port change saved via VS Code settings takes effect
        if (!anthropicServerInstance?.isRunning) {
            const latestPort = vscode.workspace.getConfiguration('amazonQ').get('anthropicServer.port', 61823);
        ...
        const cfg = vscode.workspace.getConfiguration('amazonQ');
        const currentPort = cfg.get('anthropicServer.port', 61823);
        const currentAutoStart = cfg.get('anthropicServer.autoStart', true);
        const currentDockerEnabled = cfg.get('anthropicServer.dockerEnabled', false);
        const currentDockerImage = cfg.get('anthropicServer.defaultEnvironmentImage', 'ubuntu:24.04');
        const currentMemoryMb = cfg.get('anthropicServer.containerMemoryMb', 512);
        const currentRetry = cfg.get('anthropicServer.retryOnPortBusy', true);
        const currentRetryInterval = cfg.get('anthropicServer.retryIntervalSeconds', 5);
        const running = anthropicServerInstance?.isRunning ?? false;
        anthropicSettingsPanel = vscode.window.createWebviewPanel('amazonq.anthropicServerSettings', 'Anthropic-Compatible Server', vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true });
        anthropicSettingsPanel.webview.html = buildAnthropicSettingsHtml(anthropicSettingsPanel, running, currentPort, currentAutoStart, currentDockerEnabled, currentDockerImage, currentMemoryMb, currentRetry, currentRetryInterval);
    ...
    if (autoStart) {
        anthropicServerInstance.start().catch((err) => serverUtils_1.log.error('Anthropic server auto-start failed: %s', err));
    }
}
exports.activateAnthropicServer = activateAnthropicServer;
